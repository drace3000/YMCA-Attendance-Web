...
export async function POST(req: Request) {
  ...
  if (instructor_ids && instructor_ids.length > 0) {
    ...
    if (linkError) {
      console.error("Error linking instructors:", linkError);
    }
  }
...
export async function PUT(req: Request) {
  const body: UpdateSessionPayload = await req.json();
  ...
  if (instructor_ids !== undefined) {
    ...
    if (deleteError) {
      console.error("Error deleting instructor links:", deleteError);
    }
