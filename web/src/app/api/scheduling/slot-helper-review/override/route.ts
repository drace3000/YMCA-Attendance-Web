  ...
  try {
    ...
    if (requestErr) {
      return await serverErrorResponse({
        req,
        errorType: "DB_ERROR",
        publicMessage: "Failed to override slot helper request",
        logMessage: requestErr.message,
        context: { module: "api.scheduling.slot_helper_review.override", action: "update_request", requestId, branchId },
        err: requestErr,
      });
    }
    ...
    if (releaseErr) {
      return await serverErrorResponse({
        ...
        context: { module: "api.scheduling.slot_helper_review.override", action: "release_holds", requestId, branchId },
        err: releaseErr,
      });
    }
    ...
    return await serverErrorResponse({
      ...
      logMessage: "Unhandled error in POST /api/scheduling/slot-helper-review/override",
      context: { module: "api.scheduling.slot_helper_review.override", action: "unhandled" },
      err,
    });
  }
}
