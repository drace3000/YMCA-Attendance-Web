...
export function EmailExcelModal({
  isOpen,
  onClose,
  excelBlob,
  defaultSubject,
  ...
  const handleSend = async () => {
    if (!excelBlob) {
      setError("Excel file not generated. Please try again.");
      return;
    }
    ...
    try {
      ...
      // Use the same email API endpoint - it works for any attachment type
  ...
  const fileSizeKB = excelBlob ? Math.round(excelBlob.size / 1024) : 0;
            ...
            <div className="mb-4 flex items-center gap-3 rounded-xl border border-[var(--brand-strong)]/50 bg-[var(--brand-strong)]/20 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[var(--brand-ink)]/60">
                  {fileSizeKB} KB • Excel attachment
                </p>
              </div>
            </div>
